...
function init_ensemble_from_python(py_ensemble, settings :: Dynamics{T}) where {T <: AbstractFloat}
    ...
    rho0 = QuantumGaussianDynamics.init_from_dyn(dyn0, Float64(TEMPERATURE), settings)
    ...
    ensemble = QuantumGaussianDynamics.Ensemble(rho0 = rho0, positions = ens_positions, forces = ens_forces, stress = ens_voigt,
