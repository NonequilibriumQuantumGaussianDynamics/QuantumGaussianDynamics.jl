...
function test_compute_force_ase()
    ...
    energy_ase = TDSCHA.compute_configuration!(forces_ase, stress, calculator, positions, wigner.masses)
    energy_jl = TDSCHA.compute_configuration!(forces_jl, stress, harmonic_calculator!, positions, wigner.masses)
...
function test_ase_calculator_harmonic()
    ...
    efield = TDSCHA.fake_field(1)

    TDSCHA.integrate!(wigner, ensemble, settings, calculator, efield)
...
function test_julia_harmonic3d()
    ...
    TDSCHA.generate_ensemble!(ensemble, wigner)
    TDSCHA.calculate_ensemble!(ensemble, harmonic_calculator!)

    efield = TDSCHA.fake_field(1)

    TDSCHA.integrate!(wigner, ensemble, settings, harmonic_calculator!, efield)
