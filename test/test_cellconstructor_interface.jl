...
function test_dyn_scha_converged()
    ...
    settings = TDSCHA.Dynamics(dt, total_t, N_configs;
                                                algorithm = algorithm,
                                                seed = 1234,
                                                save_each = 1,
    ...
    # Convert from python
    wigner = TDSCHA.init_from_dyn(dyn_py, temperature, settings)
    ...
    calc_jl = TDSCHA.init_calculator(calc_py, wigner, ATOMS_py.Atoms)
    ...
    efield = TDSCHA.fake_field(wigner.n_atoms)
    ...
    TDSCHA.calculate_ensemble!(ensemble, calc_jl!)
    ...
    TDSCHA.integrate!(wigner, ensemble, settings, calc_jl, efield)
