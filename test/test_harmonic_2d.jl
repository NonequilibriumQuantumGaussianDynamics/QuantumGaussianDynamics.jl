...
function harmonic_potential!(forces, stress, positions; ndims=2)
    ...
    @simd for i in 1:length(positions)
        ...
        forces[i] = -mass[atomic_index] * ω[i]^2 * (positions[i] - x0[i])
        energy += 0.5 * mass[atomic_index] * ω[i]^2 * (positions[i] - x0[i])^2
    end
    return energy
end
...
function test_harmonic_1_particle_2d()
    ...
    for i in 1:length(ω)
        wigner_dist.RR_corr[i,i] = 1/(2ω[i])
        wigner_dist.PP_corr[i,i] = ω[i]^2 * wigner_dist.RR_corr[i,i]
    end
    ...
    efield = QuantumGaussianDynamics.fake_field(1; ndims=2)
    ...
    @test wigner_dist.RR_corr[1,1] ≈ 1/(2ω[1]) rtol = 5e-2
    @test wigner_dist.RR_corr[2,2] ≈ 1/(2ω[2]) rtol = 5e-2
end
