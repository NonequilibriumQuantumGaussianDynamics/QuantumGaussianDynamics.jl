...
function test_cleaning(; verbose=false)
    ...
    RR, PP = TDSCHA.get_correlators(temperature, ω, eigenvecs, get_general_settings(settings))
    wigner.RR_corr .= RR
    wigner.PP_corr .= PP
    ...
    # Define the ensemble
    ensemble = TDSCHA.Ensemble(wigner, settings; n_configs=N_configs, temperature=temperature)
    if verbose
        @show raman_tensor
    end
    ...
    filter_invariant_symmetries!(symmetry_group, perturbation_direction, cell)
    ...
    TDSCHA.generate_ensemble!(N_configs, ensemble, wigner)
    TDSCHA.calculate_ensemble!(ensemble, co2_force_field!)

    # Get the external force
    ext_for = TDSCHA.get_external_forces(0.0, raman_field, wigner)
    if verbose
        @show ext_for
    end
    ...
    TDSCHA.integrate!(wigner, ensemble, settings, co2_force_field!, raman_field)

    # Check that the correlators are not changed
    for i in 1:length(wigner.RR_corr)
        @test wigner.RR_corr[i] ≈ rr_copy[i] atol = 1e-6
    end
    for i in 1:length(wigner.R_av)
        @test wigner.R_av[i] ≈ r_copy[i] atol = 1e-6
    end
end
