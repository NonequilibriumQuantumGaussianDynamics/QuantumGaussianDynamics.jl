...
function test_corr_rr_generate()
    Ncfgs = 100000
    settings = TDSCHA.Dynamics(1.0u"fs", 1.0u"fs", Ncfgs;
                                                settings = NoASR())
    ...
    ensemble = TDSCHA.Ensemble(wigner_dist, settings; n_configs=Ncfgs, temperature=0.0u"K")


    # Generate the ensemble
    TDSCHA.generate_ensemble!(Ncfgs, ensemble, wigner_dist)
    ...
    TDSCHA.get_averages!(avg_for, d2v_dr2, ensemble, wigner_dist, get_stochastic_settings(settings))
