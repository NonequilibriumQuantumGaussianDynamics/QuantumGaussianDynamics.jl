...
function test_rr_phi()
    ...
    settings = QuantumGaussianDynamics.Dynamics(0.0, 0.0, 10)
    wigner = QuantumGaussianDynamics.init_from_dyn(dyn_test, temperature, settings)

    Φ = QuantumGaussianDynamics.get_Φ(wigner, ustrip(uconvert(u"K", temperature)))
