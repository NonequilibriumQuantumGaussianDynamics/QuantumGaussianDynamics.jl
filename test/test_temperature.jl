...
function test_temperature()
    ...
    dt = 1.0u"fs"
    total_time = 80.0u"fs"
    dyn_file = joinpath(@__DIR__, "data/dyn_t500_sym_converged")
    nqirr = 1
    N_configs = 500
    ...
    settings = TDSCHA.Dynamics(dt, total_time, N_configs;
                                                algorithm = algorithm,
                                                seed = 1234,
                                                save_filename = "temp$temperature",
                                                save_each=1)

    dyn = PH.Phonons(dyn_file, nqirr)
    wigner = TDSCHA.init_from_dyn(dyn, temperature, settings)
    ...
    ensemble = TDSCHA.Ensemble(wigner, settings; 
                                                n_configs=N_configs, 
    ...
    calc! = TDSCHA.init_calculator(calc, wigner, ase.Atoms)

    efield = TDSCHA.fake_field(get_natoms(wigner))

    TDSCHA.generate_ensemble!(N_configs, ensemble, wigner)
    TDSCHA.calculate_ensemble!(ensemble, calc!)

    RR_corr_start = copy(wigner.RR_corr)

    TDSCHA.integrate!(wigner, ensemble, settings, calc!, efield;
                                       symmetry_group = symmetry_group)

    nat = get_natoms(wigner)
    for i in 1:3nat
        for j in 1:3nat
            @test wigner.RR_corr[i, j] ≈ RR_corr_start[i, j] atol=1e-8 rtol=4e-2
        end
    end
end
